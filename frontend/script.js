document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Renderer Functions (Simplified and Corrected) ---
    ...
    function renderPersonalInfo(info) {
        ...
        // ** THE FIX **: Use the URL from the API directly.
    ...
    function renderSkills(skills) {
        ...
        skills.forEach(skill => {
            ...
            // ** THE FIX **: Use the image_url directly from the API.
            if (skill.image_url) {
        ...
        memories.forEach(memory => {
            if (memory.photos && memory.photos.length > 0) {
                memory.photos.forEach(photoUrl => {
                    allPhotos.push({
                        // ** THE FIX **: The photoUrl from the API is the full Cloudinary URL.
                        url: photoUrl,
                        title: memory.title,
    ...
    // --- Unchanged Functions ---
    function renderSocialLinks(links) { /* Unchanged */ }
    function renderProjects(projects) { /* Unchanged */ }
    function initializeCarousel() { /* Unchanged */ }
    ...
    function renderSocialLinks(links) {
        ...
        links.forEach(link => { const a = document.createElement('a'); a.href = link.url; a.textContent = link.name; a.target = '_blank'; container.appendChild(a); });
    }
    function renderProjects(projects) {
        projectGrid.innerHTML = '';
        if (!projects || projects.length === 0) { projectGrid.innerHTML = '<p>No projects added yet.</p>'; return; }
        projects.forEach(project => { const card = document.createElement('div'); card.className = 'project-card'; card.innerHTML = `<h3>${project.title}</h3><p>${project.description}</p>${project.project_url ? `<a href="${project.project_url}" class="visit-button" target="_blank">Visit</a>` : ''}`; projectGrid.appendChild(card); });
    }
    function initializeCarousel() {
        const slides = document.querySelectorAll('.memory-slide'); if (slides.length <= 1) return;
        let currentIndex = 0; setInterval(() => { if (slides[currentIndex]) slides[currentIndex].classList.remove('active'); currentIndex = (currentIndex + 1) % slides.length; if (slides[currentIndex]) slides[currentIndex].classList.add('active'); }, 3000);
    }
